...
export async function POST(req: NextRequest) {
    ...
    const sanitizedUserMsg    = sanitizeUserPrompt(userMessage)
    const sanitizedFirstName  = sanitizeUserPrompt(firstName)
...
CONTEXT:
- Recipient: ${sanitizedFirstName} (user)
- Ticket subject: ${sanitizedSubject}
- Original user message: "${sanitizedUserMsg?.slice(0, 300) ?? ''}"

INSTRUCTIONS:
- ${langInstruction}
- Keep the core content and all facts of the draft — do not change any factual statements
- Address "${sanitizedFirstName}" directly (personal, not generic)
- Use a friendly, professional, and solution-oriented tone
- Remove placeholders like [user name] and replace them with "${sanitizedFirstName}"
...
KONTEXT:
- Empfänger: ${sanitizedFirstName} (Nutzer)
- Betreff des Tickets: ${sanitizedSubject}
- Originalnachricht des Nutzers: "${sanitizedUserMsg?.slice(0, 300) ?? ''}"

ANWEISUNGEN:
- ${langInstruction}
- Behalte den Kerninhalt und alle Fakten des Entwurfs bei — ändere keine inhaltlichen Aussagen
- Passe die Ansprache direkt an "${sanitizedFirstName}" an (persönlich, nicht generisch)
- Verwende einen freundlichen, professionellen und lösungsorientierten Ton
- Entferne Platzhalter wie [Name des Nutzers] und ersetze sie mit "${sanitizedFirstName}"
