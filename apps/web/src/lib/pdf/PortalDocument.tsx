...
import type { AnalysisResult, GradeResult, IndividualFlag } from '@/lib/calculations/types'
...
function hrFmt(val: number | null | undefined): string {
...
export const PortalDocument = ({ orgName, analysis, employee, gradeInfo }: PortalDocumentProps) => {
        ...
        <Document>
            <Page size="A4" style={s.page}>
                ...
                <View style={s.content}>
                    <Text style={s.title}>Auskunftsrecht (Art. 7)</Text>
                    <Text style={s.subtitle}>
                        Bereitstellung von Entgeltinformationen an Mitarbeitende gem. EU-Richtlinie 2023/970
                    </Text>

                    <View style={s.cardBox}>
                        <View style={s.cardTitleRow}>
                            <View>
                                <Text style={s.cardTitle}>Mitarbeiterinformation</Text>
                        ...
                        <Text style={s.sectionTitle}>Objektive Kriterien der Einstufung</Text>
                        <View style={s.infoBox}>
                            <Text style={s.infoTitle}>Vergleichsgruppe (Kohorte): {employee.job_grade ?? '—'}</Text>
                        ...
                        <Text style={s.sectionTitle}>Durchschnittliche Gehaltsstruktur (Bruttostundenverdienst)</Text>
                        <View style={s.table}>
                            <View style={s.tableHeaderRow}>
                                <Text style={s.col1}>Vergleichsgruppe: {employee.job_grade}</Text>
                                <Text style={{ ...s.col2, color: MUTED, fontFamily: 'Helvetica' }}>Mittelwert</Text>
                                <Text style={{ ...s.col3, color: MUTED, fontFamily: 'Helvetica' }}>Median</Text>
                            </View>
                            <View style={s.tableRow}>
                                <Text style={s.col1}>Davon Frauen</Text>
                                <Text style={s.col2}>{hrFmt(gradeInfo.gap.female_mean_salary)}</Text>
                                <Text style={s.col3}>{hrFmt(gradeInfo.gap.female_median_salary)}</Text>
                            </View>
                            <View style={s.tableRow}>
                                <Text style={s.col1}>Davon Männer</Text>
                                <Text style={s.col2}>{hrFmt(gradeInfo.gap.male_mean_salary)}</Text>
                                <Text style={s.col3}>{hrFmt(gradeInfo.gap.male_median_salary)}</Text>
                            </View>
                            <View style={s.tableRowHighlight}>
                                <Text style={{ ...s.col1, color: NAVY, fontFamily: 'Helvetica-Bold' }}>Ihr Entgelt (Analyse-Stichtag)</Text>
                                <Text style={{ flex: 2, fontSize: 10, color: BRAND, textAlign: 'right', fontFamily: 'Helvetica-Bold' }}>
                                    {hrFmt(employee.hourly_rate)}
                                </Text>
                            </View>
                        </View>

                        <Text style={s.lawRef}>
                            Nach Art. 7 der Richtlinie 2023/970 ist diese Auskunft binnen zwei Monaten nach Anfrage zu erteilen.{'\n'}
